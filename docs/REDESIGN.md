...
Package creation now supports a searchable existing-service picker, including package-only services. Create exclusive services in Admin → Services using the Package only checkbox, then refresh the package picker. Linked services appear on package cards; additional free-text inclusions remain supported. Package price and number of visits remain independent of individual service prices.

The owner's four-page printed brochure now supplies the catalogue, rates, contact number, hours and medical-team details. Removed unlisted standalone therapy promotions; programme descriptions retain only brochure-supported scope. New offerings lacking confirmed scheduling data use Call to book with the printed centre number. Multi-rate entries display both rates. Administrators can confirm duration/capacity and turn off Contact to book. The next-OPD exclusions and consultation introductory-offer caveat are visible in booking.
