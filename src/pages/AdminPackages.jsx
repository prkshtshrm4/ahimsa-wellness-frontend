...
const blank = { name: '', blurb: '', price: '', durationMin: 60, visitCount: 1, capacity: 1, inclusions: '', active: true };
...
export default function AdminPackages() {
  ...
  const edit = p => setEditing(p ? { ...p, price: (p.priceInPaise / 100).toString(), inclusions: (p.inclusions || []).join('\n') } : { ...blank });
  const set = (key, value) => setEditing(old => ({ ...old, [key]: value }));
  async function save(event) {
    event.preventDefault();
    setBusy(true);
    try {
      const body = { name: editing.name.trim(), blurb: editing.blurb.trim(), priceInPaise: Math.round(Number(editing.price) * 100), durationMin: Number(editing.durationMin), capacity: Number(editing.capacity), visitCount: Number(editing.visitCount), active: editing.active, inclusions: editing.inclusions.split('\n').map(x => x.trim()).filter(Boolean) };
    ...
    <dialog ref={dialog} className="package-editor" onCancel={e => { if (busy) e.preventDefault(); else setEditing(null); }} onClose={() => { if (!busy) setEditing(null); }}>
      {editing && <form onSubmit={save}>
        ...
        <fieldset disabled={busy}><div className="package-form-grid">
          ...
          <label className="full">Inclusions (one per line)<textarea required rows={4} value={editing.inclusions} onChange={e => set('inclusions', e.target.value)} /></label>
        ...
        </div><p className="package-editor-note">Price covers all visits. Duration and capacity apply to each appointment. Reserve dedicated capacity for the package; inclusions do not automatically reserve other therapy rooms. Existing purchases keep their original visit count and inclusions.</p><div className="package-admin-actions"><button type="button" onClick={() => setEditing(null)}>Cancel</button><button type="submit" style={s.btnPrimary}>{busy ? 'Saving…' : 'Save package'}</button></div></fieldset>
      </form>}
    </dialog>
  </div>;
}
